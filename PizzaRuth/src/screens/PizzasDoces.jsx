...
function PizzasDoces({ atualizarPedidos }) {
  ...
  const atualizarTamanho = (value) => {
    ...
    if (pizzaSelecionada) {
      switch (value) {
        case 'Pequena': novoTotal = quantidadeSelecionada * pizzaSelecionada.vl_pizza - 10; break;
        case 'Média': novoTotal = quantidadeSelecionada * pizzaSelecionada.vl_pizza; break;
        case 'Grande': novoTotal = quantidadeSelecionada * pizzaSelecionada.vl_pizza + 15; break;
        default: novoTotal = quantidadeSelecionada * pizzaSelecionada.vl_pizza; break;
      }
    }
  ...
  const CadastrarPedido = async (e) => {
    ...
    try {
      ...
      if (atualizarPedidos) atualizarPedidos();
      setNome(''); setImagem(''); setTamanho(''); setQuantidade(''); setValor('');
    } catch (error) {
  ...
  return (
    <>
      <div className="w-screen h-full overflow-x-hidden"
        ...
        <NavBar />
        <p className="text-3xl md:text-5xl mt-20 text-center text-black">Pizzas Doces</p>
            ...
            <div key={pizza.id} className="w-full max-w-xs">
              <Card className="w-full">
                <div className="flex flex-col">
                  <Typography className="text-xl self-center text-black font-normal text-center px-2">{pizza.nm_sabor}</Typography>
                  <img className="w-full h-44 object-cover self-center mt-2 rounded-lg" src={pizza.cd_img} alt="card-image" />
                  <CardBody>
          ...
          <form onSubmit={(e) => CadastrarPedido(e)}>
            <div className="flex flex-col md:flex-row mt-4 text-black">
              <div className="flex flex-col items-center mx-4">
                ...
                <Typography variant="paragraph" className="text-center mt-5 text-lg mx-2">
              ...
              <div className="flex flex-col items-center mt-4 md:mt-0">
                ...
                <div className="mt-4 text-xl">
                  ...
                  <Radio name="type" label="Media" checked={tamanhoSelecionado === 'Média'} value={tamanho} onChange={() => atualizarTamanho('Média')} />
                ...
                <div className="mt-5">
                  <input className="border border-gray-400 py-1 px-2 w-44 text-lg rounded-lg bg-white text-black"
                    placeholder="Número" type="number" name="numero" id="numero" min="1" step="1"
                    onChange={(e) => { const value = Math.max(1, e.target.value); atualizarQuantidade(value); }}
            ...
            <DialogFooter>
              <Button variant="text" color="red" onClick={() => setOpen(false)} className="mr-1 bg-white"><span>Cancelar</span></Button>
              <Button variant="gradient" color="green" type="submit" onClick={() => setOpen(false)}><span>Confirmar</span></Button>
            </DialogFooter>
          </form>
        </Dialog>
        <Footer />
