...
function CadastroPizzaDoce() {
  const [sabor, setSabor] = useState('');
  const [imagem, setImagem] = useState('');
  const [descricao, setDescricao] = useState('');
  const [valor, setValor] = useState('');
  const [pizzas_doces, setPizzas] = useState([]);
  ...
  return (
    <>
      <div className="w-screen h-full  overflow-x-hidden"
        ...
        <Typography className="text-4xl mt-20 text-center font-semibold">Pizzas Doces Cadastradas</Typography>
        <div className="flex flex-wrap mb-20 ml-6 gap-20 mt-20 justify-center">
          {pizzas_doces.map((pizza) =>
            <div key={pizza.id}>
